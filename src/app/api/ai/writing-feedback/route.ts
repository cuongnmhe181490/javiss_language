import { aiWritingFeedbackSchema } from "@/features/ai/schemas";
import { getSession } from "@/lib/auth/session";
import { AppError, getErrorMessage } from "@/lib/utils/app-error";
import { fail, ok } from "@/lib/utils/response";
import { assertHasRole } from "@/server/policies/rbac";
import { generateWritingFeedback } from "@/server/services/writing-feedback.service";
...
export async function POST(request: Request) {
  try {
    ...
    const input = aiWritingFeedbackSchema.parse(body);
    ...
    if (error instanceof Error && error.message === "RATE_LIMIT_EXCEEDED") {
      return fail(
        "Bạn đang gửi bài quá nhanh. Vui lòng chờ một chút rồi thử lại.",
        429,
        "RATE_LIMIT",
      );
    }
