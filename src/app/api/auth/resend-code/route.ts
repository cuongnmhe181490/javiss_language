import { resendCodeSchema } from "@/features/auth/schemas";
import { AppError, getErrorMessage } from "@/lib/utils/app-error";
import { fail, ok } from "@/lib/utils/response";
import { resendVerificationCode } from "@/server/services/registration.service";

export async function POST(request: Request) {
  try {
    const body = await request.json();
    const input = resendCodeSchema.parse(body);
    const result = await resendVerificationCode(input);
    return ok(result);
  } catch (error) {
