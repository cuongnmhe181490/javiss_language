    ...
    <header className="fixed inset-x-0 top-0 z-50 border-b border-border bg-background/80 backdrop-blur-2xl">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link href="/" className="flex min-h-11 items-center gap-3" aria-label="Học ngoại ngữ cùng AI" aria-current="page">
          ...
          <span className="text-sm font-semibold tracking-wide text-foreground">Học ngoại ngữ cùng AI</span>
        </Link>
        <nav className="hidden items-center gap-1 text-sm text-muted-foreground md:flex">
          <Link className="inline-flex min-h-11 items-center rounded-xl px-3 transition hover:bg-accent hover:text-foreground" href="/dashboard">Góc học</Link>
          <Link className="inline-flex min-h-11 items-center rounded-xl px-3 transition hover:bg-accent hover:text-foreground" href="/#flow">Cách học</Link>
          <Link className="inline-flex min-h-11 items-center rounded-xl px-3 transition hover:bg-accent hover:text-foreground" href="/#trust">Nguồn học</Link>
        </nav>
        <div className="flex items-center gap-2">
          <ThemeSwitcher />
          <Button size="sm" asChild className="h-11 rounded-2xl bg-primary px-4 text-primary-foreground hover:opacity-90">
      ...
      <div className="mx-auto grid min-h-[calc(100svh-4rem)] max-w-6xl items-center gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[1fr_1fr] lg:px-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: "easeOut" }}>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Button asChild size="lg" className="h-12 rounded-2xl bg-primary px-6 text-base font-medium text-primary-foreground hover:opacity-90">
              <Link href="/dashboard">Vào học thử <ArrowRight className="ml-2 size-4" /></Link>
            </Button>
            <Button asChild size="lg" variant="outline" className="h-12 rounded-2xl border-border px-6 text-base text-muted-foreground hover:bg-accent hover:text-foreground">
        ...
        >
          ...
          <div className="rounded-3xl border border-border bg-card p-6 backdrop-blur-xl">
            ...
            <p className="text-sm text-muted-foreground mb-4">Nói được một lời chào, giới thiệu ngắn và hỏi thêm một câu lịch sự.</p>
          ...
          <div className="rounded-3xl border border-border bg-card p-5 backdrop-blur-xl">
            ...
            <p className="text-sm text-muted-foreground italic">&ldquo;Chậm hơn một nhịp ở câu thứ hai — nghe tự nhiên hơn đó.&rdquo;</p>
          </div>
        </motion.div>
      </div>
    </section>
    ...
    <section id="flow" className="py-24">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          ...
          <p className="mt-2 text-muted-foreground">Không cần kế hoạch phức tạp. Mở lên, học, xong.</p>
        </motion.div>
    ...
    <section className="py-24">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="mt-3 text-muted-foreground">Không cần đăng ký. Mở dashboard, chọn bài, bắt đầu.</p>
